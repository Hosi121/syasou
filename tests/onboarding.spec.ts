...
test('first entry follows the paper guide into the notebook without starting a timer', async ({ page }) => {
  await page.goto('/')
  ...
  await page.getByRole('button', { name: '手帳をひらく', exact: true }).click()
  ...
  expect(saved.travel.tickets).toEqual([])
