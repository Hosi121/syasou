...
PC・スマートフォンのブラウザで遊べます。マウスやタッチに加え、キーボードでも操作できます。スマートフォンの「ホーム画面に追加」にも対応しています。
...
画面・操作・旅と切符のロジック・保存・WebGL 描画・音声合成まで、アプリ本体を MoonBit で実装しています。画面更新・状態管理・spring も MoonBit で実装し、ダイアログにはブラウザ標準 API を使っています。React は使っていません。生成した JavaScript を Vite でビルドし、Vercel で配信します。
