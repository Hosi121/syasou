...
画面・操作・旅と切符のロジック・保存・WebGL 描画・音声合成まで、アプリ本体を MoonBit で実装しています。画面更新・状態管理・spring も MoonBit で実装し、ダイアログにはブラウザ標準 API を使っています。React は使っていません。MoonBit を WebAssembly にコンパイルし、小さな起動用 JS とブラウザ API の接続コードとともに、Vercel から静的配信します。
