...
export default function App() {
  ...
  const sound = useSound(preferences, moving, windowOpen)
  ...
  useEffect(() => {
    const keydown = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && atWindow && !notebookOpen && !trayOpen) leaveWindow()
    }
  ...
  }, [atWindow, notebookOpen, trayOpen, leaveWindow])
  ...
  return <MotionConfig reducedMotion="user">
    <main className={cn('world', { 'at-window': atWindow, 'journal-is-open': notebookOpen, 'world-awake': awake })}
      aria-label="列車の窓辺"
      onPointerMove={event => {
        wake()
        if (reducedMotion || notebookOpen || atWindow || trayOpen) return
      ...
      <motion.div className="world-camera" style={{ x: lookX, y: lookY }}>
        ...
        <motion.div className="window-world" animate={atWindow ? { scale: 1.36, x: '13%', y: '5%' } : { scale: 1, x: '0%', y: '0%' }} transition={{ duration: reducedMotion ? 0 : 1.15, ease: 'easeInOut' }}>
          <Landscape scene={preferences.scene} speed={preferences.speed} moving={moving} windowOpen={windowOpen} travelling={journey.phase === 'focus'} />
      ...
      {trayOpen && <TicketTray key={pendingArrivalId ?? 'archive'} tickets={tickets} arrivalId={pendingArrivalId} saveError={saveError} onEdit={editTicket} onClose={() => {
